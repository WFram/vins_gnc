...
void FeaturePerId::initializeReliabilityModel(const unsigned char label) { reliability_model.initializeModel(label); }

void FeaturePerId::updateReliabilityModel(const unsigned char label) { reliability_model.updateReliability(label); }
...
bool FeatureManager::addFeatureCheckParallax(int frame_count, const FeatureFrame &image, double td) {
  ...
  new_feature_num = 0;
  long_track_num = 0;
  // TODO: since it iterates 150 times, we can set weights
  for (auto &id_pts : image) {
    ...
    if (it == feature.end()) {
      ...
      if (USE_SEMANTICS) {
        auto label = static_cast<unsigned char>(id_pts.second[0].second(7));
        feature.back().initializeReliabilityModel(label);
      }
      ...
      if (USE_SEMANTICS) {
        auto label = static_cast<unsigned char>(id_pts.second[0].second(7));
        it->updateReliabilityModel(label);
      }
...
vector<pair<Vector3d, Vector3d>> FeatureManager::getCorrespondences(int frame_count_l, int frame_count_r) {
