...
class FeaturePerFrame {
 public:
  FeaturePerFrame(const Feature &_point, double td) {
    point.x() = _point(0);
    point.y() = _point(1);
    point.z() = _point(2);
    uv.x() = _point(3);
  ...
  void rightObservation(const Feature &_point) {
...
class FeaturePerId {
 public:
  ...
  void initializeReliabilityModel(const unsigned char label);

  void updateReliabilityModel(const unsigned char label);
};
...
class FeatureManager {
 public:
  ...
  vector<pair<Vector3d, Vector3d>> getCorrespondences(int frame_count_l, int frame_count_r);
