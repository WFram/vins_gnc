...
#include "altProjectionTwoFrameOneCamFactor.h"
#include <ceres/gradient_checker.h>
#include "pose_local_parameterization.h"

Eigen::Matrix2d AltProjectionTwoFrameOneCamFactor::sqrt_info;
double AltProjectionTwoFrameOneCamFactor::sum_t;

AltProjectionTwoFrameOneCamFactor::AltProjectionTwoFrameOneCamFactor(const Eigen::Vector3d &_pts_i,
...
double AltProjectionTwoFrameOneCamFactor::ComputeResidual(double const *pPi, double const *pPj, double const *pTic,
...
bool AltProjectionTwoFrameOneCamFactor::Evaluate(double const *const *parameters, double *residuals,
  ...
  if (jacobians) {
    ...
    if (jacobians[3]) {
      ...
      jacobian_feature = sqrt_weight * reduce * ric.transpose() * Rj.transpose() * Ri * ric * pts_i_td * -1.0 /
                         (inv_dep_i * inv_dep_i);
    }
    if (jacobians[4]) {
      Eigen::Map<Eigen::Vector2d> jacobian_td(jacobians[4]);
      jacobian_td = sqrt_weight * reduce * ric.transpose() * Rj.transpose() * Ri * ric * velocity_i / inv_dep_i * -1.0 +
                    sqrt_info * velocity_j.head(2);
    }
    if (jacobians[5]) {
      Eigen::Map<Eigen::Vector2d> jacobian_weight(jacobians[5]);
...
void AltProjectionTwoFrameOneCamFactor::check(double **parameters) {
