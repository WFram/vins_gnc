...
#include "altProjectionTwoFrameTwoCamFactor.h"

Eigen::Matrix2d AltProjectionTwoFrameTwoCamFactor::sqrt_info;
double AltProjectionTwoFrameTwoCamFactor::sum_t;

AltProjectionTwoFrameTwoCamFactor::AltProjectionTwoFrameTwoCamFactor(const Eigen::Vector3d &_pts_i,
...
double AltProjectionTwoFrameTwoCamFactor::ComputeResidual(double const *pPi, double const *pPj, double const *pTic,
                                                          double const *pTic2, double const *p_inv_dep_i,
                                                          double const *p_td) {
  ...
  Eigen::Vector3d tic(pTic[0], pTic[1], pTic[2]);
  Eigen::Quaterniond qic(pTic[6], pTic[3], pTic[4], pTic[5]);

  Eigen::Vector3d tic2(pTic2[0], pTic2[1], pTic2[2]);
  Eigen::Quaterniond qic2(pTic2[6], pTic2[3], pTic2[4], pTic2[5]);

  double inv_dep_i = p_inv_dep_i[0];
  double td = p_td[0];
  ...
  Eigen::Vector2d residual, sqrt_residual;
...
bool AltProjectionTwoFrameTwoCamFactor::Evaluate(double const *const *parameters, double *residuals,
  ...
  if (jacobians) {
    ...
    if (jacobians[2]) {
      ...
      jacobian_ex_pose.leftCols<6>() = sqrt_weight * reduce * jaco_ex;
    ...
    if (jacobians[3]) {
      ...
      jacobian_ex_pose1.leftCols<6>() = sqrt_weight * reduce * jaco_ex;
...
#if 1
      jacobian_feature = sqrt_weight * reduce * ric2.transpose() * Rj.transpose() * Ri * ric * pts_i_td * -1.0 /
                         (inv_dep_i * inv_dep_i);
#else
    ...
    if (jacobians[5]) {
      ...
      jacobian_td =
          sqrt_weight * reduce * ric2.transpose() * Rj.transpose() * Ri * ric * velocity_i / inv_dep_i * -1.0 +
          sqrt_info * velocity_j.head(2);
    }

    if (jacobians[6]) {
      Eigen::Map<Eigen::Vector2d> jacobian_weight(jacobians[6]);
...
void AltProjectionTwoFrameTwoCamFactor::check(double **parameters) {
