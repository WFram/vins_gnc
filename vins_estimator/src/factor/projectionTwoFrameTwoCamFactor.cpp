...
Eigen::Matrix2d ProjectionTwoFrameTwoCamAltFactor::sqrt_info;
double ProjectionTwoFrameTwoCamFactor::sum_t;
double ProjectionTwoFrameTwoCamAltFactor::sum_t;
...
ProjectionTwoFrameTwoCamAltFactor::ProjectionTwoFrameTwoCamAltFactor(const Eigen::Vector3d &_pts_i,
                                                                     const Eigen::Vector3d &_pts_j,
                                                                     const Eigen::Vector2d &_velocity_i,
...
bool ProjectionTwoFrameTwoCamAltFactor::Evaluate(double const *const *parameters, double *residuals,
                                                 double **jacobians) const {
  TicToc tic_toc;
  Eigen::Vector3d Pi(parameters[0][0], parameters[0][1], parameters[0][2]);
  Eigen::Quaterniond Qi(parameters[0][6], parameters[0][3], parameters[0][4], parameters[0][5]);
  ...
  residual = weight * sqrt_info * residual;
  ...
  if (jacobians) {
    ...
    if (jacobians[0]) {
      ...
      jacobian_pose_i.leftCols<6>() = weight * reduce * jaco_i;
      jacobian_pose_i.rightCols<1>().setZero();
    }
    ...
    if (jacobians[1]) {
      ...
      jacobian_pose_j.leftCols<6>() = weight * reduce * jaco_j;
      jacobian_pose_j.rightCols<1>().setZero();
    }
    if (jacobians[2]) {
      Eigen::Map<Eigen::Matrix<double, 2, 7, Eigen::RowMajor>> jacobian_ex_pose(jacobians[2]);
      ...
      jaco_ex.leftCols<3>() = ric2.transpose() * Rj.transpose() * Ri;
      jaco_ex.rightCols<3>() = ric2.transpose() * Rj.transpose() * Ri * ric * -Utility::skewSymmetric(pts_camera_i);
      jacobian_ex_pose.leftCols<6>() = weight * reduce * jaco_ex;
      jacobian_ex_pose.rightCols<1>().setZero();
    }
    if (jacobians[3]) {
      Eigen::Map<Eigen::Matrix<double, 2, 7, Eigen::RowMajor>> jacobian_ex_pose1(jacobians[3]);
      ...
      jaco_ex.leftCols<3>() = -ric2.transpose();
      jaco_ex.rightCols<3>() = Utility::skewSymmetric(pts_camera_j);
      jacobian_ex_pose1.leftCols<6>() = weight * reduce * jaco_ex;
      jacobian_ex_pose1.rightCols<1>().setZero();
    }
    if (jacobians[4]) {
      Eigen::Map<Eigen::Vector2d> jacobian_feature(jacobians[4]);
#if 1
      jacobian_feature =
          weight * reduce * ric2.transpose() * Rj.transpose() * Ri * ric * pts_i_td * -1.0 / (inv_dep_i * inv_dep_i);
#else
      jacobian_feature = reduce * ric.transpose() * Rj.transpose() * Ri * ric * pts_i;
#endif
    }
    if (jacobians[5]) {
      Eigen::Map<Eigen::Vector2d> jacobian_td(jacobians[5]);
      jacobian_td = weight * reduce * ric2.transpose() * Rj.transpose() * Ri * ric * velocity_i / inv_dep_i * -1.0 +
                    sqrt_info * velocity_j.head(2);
    }
    if (jacobians[6]) {
      Eigen::Map<Eigen::Vector2d> jacobian_weight(jacobians[6]);
      jacobian_weight = sqrt_info * ((pts_camera_j / dep_j).head<2>() - pts_j_td.head<2>());
    }
  }
  sum_t += tic_toc.toc();
...
void ProjectionTwoFrameTwoCamAltFactor::check(double **parameters) {
  double *res = new double[15];
  double **jaco = new double *[6];
  jaco[0] = new double[2 * 7];
  jaco[1] = new double[2 * 7];
